...
list_wolves = []
list_rabbits = []
with open('/Users/filippocasari/Dropbox/Mac/Documents/ParticleMethods/Assignment2Python/best_params.json', 'r') as f:
...
ax2.grid(True)
if (name_plot != None and os.path.exists("output_main")):
...
def animate(n):
    ...
    if (n == len(list_num_rabbits)-1 and name_plot != None and os.path.exists("output_main")):
        fig3.savefig(f"output_main/{name_plot}_populations.png",
                     dpi=300, bbox_inches='tight', pad_inches=0, format='png')
    return line,


plt.scatter(list_num_rabbits[0], list_num_wolves[0],
            c='r', label="Starting point")
plt.scatter(list_num_rabbits[-1],
            list_num_wolves[-1], c='b', label="End point")
plt.legend()
...
anim = FuncAnimation(fig, animate, frames=len(
    list_num_rabbits), interval=25, blit=True)
plt.show()
